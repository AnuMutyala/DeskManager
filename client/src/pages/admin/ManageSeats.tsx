...
import { InsertSeat, insertSeatSchema, Seat, SeatType } from "@shared/schema";
import { format } from "date-fns";
import { Ban, Pencil, Pin, Plus, RefreshCcwDot, Trash2 } from "lucide-react";
...
export default function ManageSeats() {
  ...
  const dragRef = useRef<
    ...
    | {
        ids: number[];
        offsets: Record<number, { offsetX: number; offsetY: number }>;
      }
    | null
  >(null);
  ...
  const handleConfirmBlock = () => {
    ...
    const startDate = blockStartDate
      ? format(blockStartDate, "yyyy-MM-dd")
      : today;
  ...
  const resetLayout = async () => {
    ...
    const clusters = [
      {
        labels: ["T56", "T55", "T54", "T53", "T49", "T50", "T51", "T52"],
        cols: 4,
        baseX: 20,
        baseY: topY,
      },
      {
        labels: ["T48", "T47", "T46", "T45", "T41", "T42", "T43", "T44"],
        cols: 4,
        baseX: 20 + clusterGap,
        baseY: topY,
      },
      {
        labels: ["T60", "T61", "T59", "T62", "T58", "T63", "T57", "T64"],
        cols: 2,
        baseX: 20 + clusterGap * 2,
        baseY: topY,
      },
      {
        labels: ["T68", "T69", "T67", "T70", "T66", "T71", "T65", "T72"],
        cols: 2,
        baseX: 20 + clusterGap * 3,
        baseY: topY,
      },
      {
        labels: ["T76", "T77", "T75", "T78", "T74", "T79", "T73", "T80"],
        cols: 2,
        baseX: 20 + clusterGap * 4,
        baseY: topY,
      },
      { labels: ["S4", "S3", "S2", "S1"], cols: 1, baseX: 20, baseY: bottomY },
      {
        labels: ["T8", "T9", "T7", "T10", "T6", "T11", "T5", "T12"],
        cols: 2,
        baseX: 20 + 120,
        baseY: bottomY,
      },
      {
        labels: ["T16", "T17", "T15", "T18", "T14", "T19", "T13", "T20"],
        cols: 2,
        baseX: 20 + 320,
        baseY: bottomY,
      },
    ...
    try {
      const payload = seats.map((s) => {
        const pos = positionsRef.current[s.id] ??
          positions[s.id] ?? { x: s.x ?? 0, y: s.y ?? 0 };
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body?.message || res.statusText || "Failed to save default layout",
        );
      }
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: err?.message || "Failed to save default layout",
      });
    }
  };
  ...
  const startDrag = (seat: Seat, e: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (isToggle) {
      newSelected = selectedIds.includes(seat.id)
    ...
    if (group) {
      ...
      const current = positionsRef.current[seat.id] || {
        x: seat.x ?? 0,
        y: seat.y ?? 0,
      };
    ...
    const handleMove = (ev: PointerEvent) => {
      ...
      if ("id" in drag) {
        const x = Math.max(
          0,
          Math.round(ev.clientX - rect.left - drag.offsetX),
        );
        ...
        drag.ids.forEach((id) => {
          ...
          const x = Math.max(
            0,
            Math.round(ev.clientX - rect.left - off.offsetX),
          );
          const y = Math.max(
            0,
            Math.round(ev.clientY - rect.top - off.offsetY),
          );
          next[id] = { x, y };
        });
    ...
    const handleUp = () => {
      ...
      if ("id" in drag) {
        const pos =
          liveDragPositionsRef.current[drag.id] ??
        ...
        drag.ids.forEach((id) => {
          const pos =
            liveDragPositionsRef.current[id] ?? positionsRef.current[id];
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Add, edit, or block seats in the office layout.
          </p>
        </div>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value={SeatType.REGULAR}>
                            {" "}
                            Monitor
                          </SelectItem>
                          <SelectItem value={SeatType.STANDING}>
                            No Monitor
                          </SelectItem>
                        </SelectContent>
                      </Select>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Select date range for blocking. If no dates are selected, the seat
              will be blocked for 1 day (today only).
            </p>
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    initialFocus
                    disabled={(date) =>
                      blockStartDate ? date < blockStartDate : false
                    }
                  />
                </PopoverContent>
              </Popover>
      ...
      <div className="bg-card rounded-xl border border-border/50 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-border/50 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Drag seats to arrange the floor plan.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <Pin className="w-4 h-4" /> Save Current as Default
            </Button>
            ...
            <Button
              ...
              onClick={resetLayout}
            >
              <RefreshCcwDot className="w-4 h-4" /> Reset to Default
            </Button>
          </div>
        </div>
        <div className="p-4">
          <div
            ...
            className="relative h-[700px] min-w-[1200px] rounded-lg border border-dashed border-border/60 bg-muted/20 overflow-auto"
          >
            {(() => {
              const seatSize = 48; // matches w-12 h-12
              const padding = 40;
              const allPositions = (seats || []).map(
                (seat) =>
                  positions[seat.id] || { x: seat.x ?? 0, y: seat.y ?? 0 },
              );
              const maxX = allPositions.length
                ? Math.max(...allPositions.map((p) => p.x))
                : 0;
              const maxY = allPositions.length
                ? Math.max(...allPositions.map((p) => p.y))
                : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        {seat.type === SeatType.REGULAR && (
                          <div className="absolute -top-2 left-1/2 -translate-x-1/2 text-xs bg-card/90 px-1 rounded-md border border-border/50">
                            🖥️
                          </div>
                        )}
                        ...
                        <button
                          onPointerDown={(e) => {
                            ...
                            e.preventDefault();
                            setOpenSeatMenu(
                              openSeatMenu === seat.id ? null : seat.id,
                            );
                          }}
                        ...
                        {openSeatMenu === seat.id && (
                          <div
                            className="absolute left-full top-0 ml-1 w-auto bg-card border rounded-md shadow-lg z-50 flex gap-1 p-1"
                          ...
                          >
                            ...
                            >
                              <Ban
                                className={`w-4 h-4 ${seat.isBlocked ? "text-green-600" : "text-orange-500"}`}
                              />
                            </Button>
                            <Button
                              variant="ghost"
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    This will permanently delete seat{" "}
                                    {seat.label} and remove all associated
                                    history.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                    ...
                    <div className="space-y-1">
                      ...
                      {seat.blockStartDate && seat.blockEndDate && (
                        <div className="text-xs text-muted-foreground">
                          {format(new Date(seat.blockStartDate), "MMM d")} -{" "}
                          {format(new Date(seat.blockEndDate), "MMM d, yyyy")}
                        </div>
                      )}
                    </div>
                  ) : (
